...
if __name__ == '__main__':
    ...
    parser.add_argument('-img_dir', type=str, default='./image/img', help='input dataset directory')
    parser.add_argument('-model_path', type=str, default='./models3/model-1/model-1_best.pt', help='trained model path')
    parser.add_argument('-model_type', type=str, default='resnet34', choices=['vgg16', 'resnet101', 'resnet34'])
    parser.add_argument('-out_viz_dir', type=str, default='./r3_vis', required=False, help='visualization output dir')
    parser.add_argument('-out_pred_dir', type=str, default='./r3_pred', required=False, help='prediction output dir')
    ...
    for path in tqdm(paths):
        ...
        _img_0 = Image.open(str(path))
        _img_0 = np.asarray(_img_0)
        img_0 = cv.merge([_img_0,_img_0,_img_0])
        ...
        f_count+=1
        if f_count%100==0:
            print('Averaged Execution time:', np.mean(time_list), 'seconds')
        if args.out_pred_dir != '':
            out_path = path.stem[0:len(path.stem)-3]+'annot'
            cv.imwrite(filename=join(args.out_pred_dir, f'{out_path}.jpg'), img=(prob_map_full * 255).astype(np.uint8))
